...
def get_args_parser():
    ...
    # Loss coefficients
    parser.add_argument('--noun_loss_coef', default=4, type=float)
    parser.add_argument('--verb_loss_coef', default=1, type=float)
    parser.add_argument('--bbox_loss_coef', default=10, type=float)
    parser.add_argument('--bbox_conf_loss_coef', default=10, type=float)
...
def main(args):
    ...
    # For Preprocessing
    if args.preprocess == True:
        batch_preprocess_sampler_train = torch.utils.data.BatchSampler(preprocess_sampler_train, args.batch_size, drop_last=False)
        batch_preprocess_sampler_val = torch.utils.data.BatchSampler(preprocess_sampler_val, args.batch_size, drop_last=False)
        batch_preprocess_sampler_test = torch.utils.data.BatchSampler(preprocess_sampler_test, args.batch_size, drop_last=False)
    ...
    print("Start training GSRFormer Encoder at epoch ",args.encoder_start_epoch)
    ...
    print("Start training GSRFormer Decoder at epoch ",args.decoder_start_epoch)
    start_time = time.time()
