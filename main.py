...
def main(args):
    ...
    if args.dev or args.test:
        ...
        # build dataset
        if args.dev:
            with open("__storage__/ValidationDict.json") as val_json:
                val_dict = json.load(val_json)
            processed_dataset_val = build_processed_dataset(image_set='Validation', args=args, neighbors_dict=val_dict)
            if args.distributed:
    ...
    if args.preprocess == True:
        ...
        preprocess_neighbors(encoder_model, preprocess_data_loader_val,
                             "val", device, args.images_per_segment)
        preprocess_neighbors(encoder_model, preprocess_data_loader_test,
                             "test", device, args.images_per_segment)
