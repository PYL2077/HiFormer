...
class GSRFormer_Encoder(nn.Module):
    ...
    def __init__(self, backbone, transformer1, transformer2, vidx_ridx):
    ...
    def forward(self, samples, targets=None, inference=False):
        ...
        for i in range(batch_size):
            ...
            verb_ft, noun_ft = self.transformer2(img_ft, verb_ft, verb_pred_1, 
...
class SWiG_Criterion_Encoder(nn.Module):
    ...
    def forward(self, outputs, targets, eval=False):
        ...
        verb_2_loss = self.loss_function_verb(verb_pred_logits, gt_verbs)

        verb_pred_logits = outputs['pred_verb_1'].squeeze(1)
        gt_verbs = torch.stack([t['verbs'] for t in targets])
        verb_1_acc_topk = accuracy(verb_pred_logits, gt_verbs, topk=(1, 5))
        verb_1_loss = self.loss_function_verb(verb_pred_logits, gt_verbs)
...
def build_gsrformer_encoder(args):
    ...
    model = GSRFormer_Encoder(backbone, transformer1, transformer2, vidx_ridx=args.vidx_ridx)
    criterion = None
