...
from .transformer import build_transformer_encoder_left, build_transformer_encoder_right
from .transformer import KNN, build_transformer_decoder
...
class GSRFormer_Decoder(nn.Module):
    def __init__(self, transformer, num_noun_classes, vidx_ridx):
        ...
        self.bbox_conf_predictor = nn.Sequential(nn.Linear(hidden_dim, hidden_dim*2),
                                             ...
                                             nn.Linear(hidden_dim*2, 1))
        self.norm1 = nn.LayerNorm(hidden_dim)
    def forward(self, batch_features, batch_role_tokens, idx=None, inference=None):
        ...
        for i in range(batch_size):
            if inference is None:
                verb_ft, noun_ft = self.transformer(batch_features[i:i+1, :, :],
                                                    batch_role_tokens[i:i+1, :, :],
                                                    idx=idx[i]).split([1,m], dim=1)
            else: 
                verb_ft, noun_ft = self.transformer(batch_features[i:i+1, :, :],
                                                    batch_role_tokens[i:i+1, :, :],
                                                    inference=inference).split([1,m], dim=1)
...
def build_gsrformer_encoder(args):
    ...
    transformer1 = build_transformer_encoder_left(args)
    transformer2 = build_transformer_encoder_right(args)

    model = GSRFormer_Encoder(backbone, transformer1, transformer2,
...
def build_gsrformer_decoder(all_features, all_role_tokens, args):
    transformer = build_transformer_decoder(all_features, all_role_tokens, args)

    model = GSRFormer_Decoder(transformer, args.num_noun_classes, args.vidx_ridx)
    criterion = None
