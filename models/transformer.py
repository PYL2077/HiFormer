...
class TransformerDecoder(nn.Module):
    def __init__(self, all_features, all_role_tokens, dim_model=512, nhead=8, num_dec_layers=6,
                 dim_feedforward=2048, dropout=0.15, activation="relu"):
        super().__init__()
        self.dim_model = dim_model
        self.nhead = nhead
        self.num_verb_classes = 504
        decoder_layer = Decoder_Layer(dim_model, nhead, dim_feedforward, dropout, activation)
        self.decoder = Decoder(decoder_layer, num_dec_layers)
        all_verb_features, all_noun_features = all_features.split([1,6], dim=1)
        self.knn = KNN(all_verb_features, all_noun_features, all_role_tokens)
        self._reset_parameters()
    def _reset_parameters(self):
        for p in self.parameters():
            if p.dim()>1: 
                nn.init.xavier_uniform_(p)
    def forward(self, features, role_tokens, idx=None, inference=None):
        bs,m = features.shape[0],features.shape[1]-1
        device = features.device
        assert bs==1
        assert m==6
        if inference is None: 
            support_features, support_role_tokens = self.knn.query(idx)
        else:
            support_features, support_role_tokens = self.knn.infer(features)
        features = torch.cat((features, support_features), dim=0)
        role_tokens = torch.cat((role_tokens, support_role_tokens), dim=0)
...
class KNN():
    """
    A datastructure that maintains the Top-K Nearest neighbors
    Metric: Cosine Similarity
    Scope: The entire training set
    Can only be solved by manual brute force
    """
    def __init__(self, verb_features, noun_features, role_tokens, num_neighbors=5, metric="cosine"):
        self.device = verb_features.device
        self.num_neighbors = num_neighbors
        
        self.verb_features=verb_features
        self.noun_features=noun_features
        self.role_tokens = role_tokens

        bs, m, dim = noun_features.shape
        self.nbrs = torch.zeros((bs, num_neighbors), dtype = int)-1
        cos_sim = nn.CosineSimilarity(dim=0)
        for i in range(bs):
            similarity = torch.zeros((bs),dtype=float)
            for j in range(bs):
                tot = 0.0
                for k in range(m):
                    tot += cos_sim(noun_features[i,k,:], noun_features[j,k,:])
                tot /= (1.0*m)
                similarity[j] = tot
            val, id = torch.topk(similarity, num_neighbors)
            id, _temp_ = torch.sort(id)
            for j in range(num_neighbors-1):
                assert id[j]<id[j+1]
            self.nbrs[i] = id
    def query(self, pos):
        # Exclusively for Training
        # return the indices of pos's K nearest neighbors
        features, role_tokens = torch.zeros(0, device=self.device), torch.zeros(0, device=self.device)
        for i in range(self.num_neighbors):
            p = self.nbrs[pos,i]
            features = torch.cat((features, torch.cat((self.verb_features[p:p+1], self.noun_features[p:p+1]), dim=1)), dim=0)
            role_tokens = torch.cat((role_tokens, self.role_tokens[p:p+1]), dim=0)
        return features, role_tokens

    def infer(self, infer_features):
        # Exclusively for inference and evaluation
        bs, m, dim = self.noun_features.shape
        similarity = torch.zeros((bs), dtype=float)
        cos_sim = nn.CosineSimilarity(dim=0)
        infer_verb_ft, infer_noun_ft = infer_features.split([1,m], dim=1)
        for i in range(bs):
            tot = 0.0
            for j in range(m):
                tot += cos_sim(infer_noun_ft[0, j, :], self.noun_features[i, j, :])
            tot /= (1.0*m)
            similarity[i] = tot
        val, id = torch.topk(similarity, self.num_neighbors)
        id, _temp_ = torch.sort(id)
...
def build_transformer_decoder(all_features, all_role_tokens, args):
    return TransformerDecoder(all_features, all_role_tokens, args.hidden_dim, args.nheads,
